...
class PopupTermsPage extends StatelessWidget {
  ...
  Widget build(BuildContext context){
    ...
    return Scaffold(
      ...
      body: Container(
        ...
        child: Center(
          child: Consumer<PopupTermsViewModel>(builder: (context, viewModel, child) {
            return Container(
                width: width * 0.9,
                height: height * 0.9,
  ...
  Widget _title(PopupTermsViewModel viewModel){
    return Container(
      ...
      child: Text(
        viewModel.agreeFlag ? '本アプリを利用するにあたり、下記の利用規約に同意していただく必要があります。ご確認の上「本規約に同意する」ボタンを押してください。' : '',
      ),
    );
  }
  ...
  Widget _webview(PopupTermsViewModel viewModel, double width, double height){
    return Container(
      ...
      decoration: const BoxDecoration(
          color: Colors.black),
      child: const WebView(
        initialUrl: 'https://firebasestorage.googleapis.com/v0/b/tuskmum-flutter.appspot.com/o/terms%2Fterms_privacy_policyhtml.html?alt=media&token=bd887e99-542b-482b-8b2c-38630b1ae350',
